...
from modules.position_encoder import TimeEncoder, SensorEncoder, TrajEncoder
...
from modules.backbone import get_embedder, Embedder
...
class MT3V2(nn.Module):
    def __init__(self, params):
        ...
        self.use_nerf = params.arch.use_nerf

        self.time_encoder = TimeEncoder(params.data_generation.n_timesteps, params.arch.d_model)
        self.sensor_encoder = SensorEncoder(params.data_generation.n_sensors, params.arch.d_model)
        self.traj_encoder = TrajEncoder(params.data_generation.max_objects, params.arch.d_model)

        # Normalization factor to make all measurement dimensions have similar standard deviations
        self.measurement_normalization_factor = params.data_generation.field_of_view.max_range - params.data_generation.field_of_view.min_range
...
############################################################################################################################################################################################################
        if self.use_nerf:
            ...
            embed_fn, input_ch = get_embedder(params.arch.multires, 0, self.d_detections)
            self.embed_fn = embed_fn

            self.dim_proj = nn.Linear(input_ch,params.arch.d_model, bias=False)
############################################################################################################################################################################################################
    ...
    def get_two_stage_proposals(self, measurement_batch, embeddings):
        ...
        measurements = measurement_batch.tensors[:, :, :2]

        xy_measurements = measurements
######## Normalize measurements to 0.25 - 0.75 (to avoid extreme regions of the sigmoid)
######## This line is the start ogf the MT3
...
###################### equation (16) in the paper
        ...
        if scores[..., 0].size(dim=1) > self.num_queries:
            topk_scores_indices = torch.topk(scores[..., 0], self.num_queries, dim=1)[1]
        else:
            topk_scores_indices = torch.topk(scores[..., 0], scores[..., 0].size(dim=1), dim=1)[1]
...
####### Compute object queries and their positional encodings by feeding the top-k memory through FFN+LayerNorm, equation (17) in the paper
    ...
    def forward(self, measurements: NestedTensor):
        ...
        if self.use_nerf:
	        input_pts = measurements.tensors[:,:,:self.d_detections]/self.measurement_normalization_factor
	        input_pts_flat = torch.reshape(input_pts, [-1, input_pts.shape[-1]])
	        embedded = self.embed_fn(input_pts_flat)
	        embedded = self.dim_proj(embedded)
	        preprocessed_measurements = torch.reshape(embedded,list(input_pts.shape[:-1]) + [embedded.shape[-1]])
        else:
            preprocessed_measurements = self.preprocessor(measurements.tensors[:,:,:self.d_detections])

        mapped_time_idx = torch.round(measurements.tensors[:,:,self.d_detections] / self.params.data_generation.dt)
        mapped_sensor_idx = torch.round(measurements.tensors[:,:,self.d_detections + 1])
        mapped_traj_idx = torch.round(measurements.tensors[:,:,self.d_detections + 2])

        time_encoding = self.time_encoder(mapped_time_idx.long())
        sensor_encoding = self.sensor_encoder(mapped_sensor_idx.long())
        traj_ids_encoding = self.traj_encoder(mapped_traj_idx.long())
        ...
        sensor_encoding = sensor_encoding.permute(1, 0, 2)
        traj_ids_encoding = traj_ids_encoding.permute(1, 0, 2)

        # Feed measurements through encoder
        embeddings = self.encoder(preprocessed_measurements, src_key_padding_mask=mask, pos=time_encoding+sensor_encoding+traj_ids_encoding)
        ...
        result = self.decoder(object_queries, embeddings,
                              ...
                              encoder_embeddings_positional_encoding=time_encoding+sensor_encoding+traj_ids_encoding,
        ...
        #print('The shape of the debug_dict is',debug_dict.shape)
